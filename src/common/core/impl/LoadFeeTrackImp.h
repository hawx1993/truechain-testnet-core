...
#ifndef SKYWELL_CORE_LOADFEETRACKIMP_H_INCLUDED
#define SKYWELL_CORE_LOADFEETRACKIMP_H_INCLUDED
...
public:
    ...
    Json::Value getJson (std::uint64_t baseFee, std::uint32_t referenceFeeUnits)
    {
...

        {
            ...
            // base_fee = The cost to send a "reference" transaction under no load, in millionths of a Skywell
            j[jss::base_fee] = Json::Value::UInt (baseFee);

            // load_fee = The cost to send a "reference" transaction now, in millionths of a Skywell
