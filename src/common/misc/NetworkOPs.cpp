...
public:
    ...
    */
    ...
    int findTransactionsByDestination (
        ...
        SkywellAddress const& destinationAccount,
    ...
    AccountState::pointer getAccountState (
        Ledger::ref lrLedger, SkywellAddress const& accountID);
    ...
    Json::Value getOwnerInfo (
        Ledger::pointer lpLedger, SkywellAddress const& naAccount);
    ...
    void processTrustedProposal (
        ...
        SkywellAddress nodePublic, uint256 checkLedger, bool sigGood);
    ...
    */
    ...
    void storeProposal (
        LedgerProposal::ref proposal, SkywellAddress const& peerPublic);
    ...
    std::string transactionsSQL (
        std::string selection, SkywellAddress const& account,
    ...
    AccountTxs getTxsAccount (
        SkywellAddress const& account, std::int32_t minLedger,
    ...
    std::vector<SkywellAddress> getLedgerAffectedAccounts (
    ...
    void subAccount (
        ...
        const hash_set<SkywellAddress>& vnaAccountIDs, bool rt);
    void unsubAccount (
        InfoSub::ref ispListener,
...
private:
    ...
    Json::Value pubBootstrapAccountInfo (
        Ledger::ref lpAccepted, SkywellAddress const& naAccountID);
...
int NetworkOPsImp::findTransactionsByDestination (
    ...
    SkywellAddress const& destinationAccount, std::uint32_t startLedgerSeq,
        ...
        do
        {
            for (auto const& uDirEntry : sleNode->getFieldV256 (sfIndexes))
            {
                ...
                case ltSKYWELL_STATE:
                    if (!jvObjects.isMember (jss::truechain_lines))
                    {
                        jvObjects[jss::truechain_lines] =
                                Json::Value (Json::arrayValue);
                    }

                    jvObjects[jss::truechain_lines].append (sleCur->getJson (0));
                    break;
...
void NetworkOPsImp::processTrustedProposal (
    LedgerProposal::pointer proposal,
    std::shared_ptr<protocol::TMProposeSet> set, SkywellAddress nodePublic,
...
Json::Value NetworkOPsImp::getServerInfo (bool human, bool admin)
{
    ...
    if (lpClosed)
    {
        ...
        SkywellAddress   accountIDMng;
        SkywellAddress   accountIDIssuer;
...
void NetworkOPsImp::unsubAccount (
    InfoSub::ref isrListener,
    hash_set<SkywellAddress> const& vnaAccountIDs,
    bool rt)
{
    for (auto const& naAccountID : vnaAccountIDs)
    {
...
{ // CAUTION: This is the old get book page logic
    ...
    auto uTransferRate = truechainTransferRate (lesActive, book.out.account);
