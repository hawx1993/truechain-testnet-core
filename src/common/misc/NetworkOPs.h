...
#ifndef SKYWELL_APP_MISC_NETWORKOPS_H_INCLUDED
#define SKYWELL_APP_MISC_NETWORKOPS_H_INCLUDED
...
#include <network/truechain.pb.h>
...
/** Provides server functionality for clients.
    ...
    truechain / NetworkOPs. However, client software connecting to non-local
    instances of truechain will need to be hardened to protect against hostile
...
public:
    ...
    virtual AccountState::pointer getAccountState (Ledger::ref lrLedger,
        SkywellAddress const& accountID) = 0;
    ...
    virtual Json::Value getOwnerInfo (Ledger::pointer lpLedger,
        SkywellAddress const& naAccount) = 0;
    ...
    virtual void storeProposal (LedgerProposal::ref proposal,
        SkywellAddress const& peerPublic) = 0;
    ...
    virtual std::string transactionsSQL (std::string selection,
        SkywellAddress const& account, std::int32_t minLedger, std::int32_t maxLedger,
    ...
    virtual MetaTxsList getAccountTxsB (SkywellAddress const& account,
        std::int32_t minLedger, std::int32_t maxLedger,  bool descending,
            std::uint32_t offset, int limit, bool bAdmin) = 0;

    virtual MetaTxsList getTxsAccountB (SkywellAddress const& account,
        std::int32_t minLedger, std::int32_t maxLedger,  bool forward,
        Json::Value& token, int limit, bool bAdmin) = 0;

    virtual std::vector<SkywellAddress> getLedgerAffectedAccounts (
