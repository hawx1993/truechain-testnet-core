...
{
    return mType == ltSKYWELL_STATE;
}

SkywellAddress STLedgerEntry::getOwner ()
{
    return getFieldAccount (sfAccount);
}

SkywellAddress STLedgerEntry::getFirstOwner ()
{
    return SkywellAddress::createAccountID (getFieldAmount (sfLowLimit).getIssuer ());
}

SkywellAddress STLedgerEntry::getSecondOwner ()
{
    return SkywellAddress::createAccountID (getFieldAmount (sfHighLimit).getIssuer ());
}
