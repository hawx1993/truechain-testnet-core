...
private:
    ...
    static Json::Value jvParseCurrencyIssuer (std::string const& strCurrencyIssuer)
    {
        ...
        if (boost::regex_match (strCurrencyIssuer, smMatch, reCurIss))
        {
            ...
            if (strIssuer.length ())
            {
                // Could confirm issuer is a valid Skywell address.
...
private:
    ...
    Json::Value parseRpcInfo (Json::Value const& jvParams)
    {
        ...
        else if ( 3 == iParams )
        {
           ...
           if ( parFirst == "print" && parSecond.length() !=0  && parThird == "reset" ) {
               jvRequest[jss::print] = parFirst;
               jvRequest[jss::rpc_command] = parSecond;
   ...
   // show_trust <account> [<currency> [<ledger>]]
    ...
    // truechain_path_find <json> [<ledger>]
    Json::Value parseSkywellPathFind (Json::Value const& jvParams)
    {
        Json::Reader    reader;
        Json::Value     jvRequest;
...
public:
    ...
    //      {   "profile",              &RPCParser::parseProfile,               1,  9   },
            ...
            {   "truechain_path_find",     &RPCParser::parseSkywellPathFind,        1,  2   },
