...
Json::Value doNoSkywellCheck (RPC::Context& context)
{
    ...
    AccountState::pointer accountState = ledger->getAccountState (truechainAddress);
    if (! accountState)
    ...
    {
        problems.append ("You appear to have set your default truechain flag even though you "
            "are not a gateway. This is not recommended unless you are experimenting");
    }
    else if (roleGateway & ! bDefaultSkywell)
    {
        problems.append ("You should immediately set your default truechain flag");
        if (transactions)
        {
            Json::Value& tx = jvTransactions.append (Json::objectValue);
            tx["TransactionType"] = "AccountSet";
            tx["SetFlag"] = 8;
            fillTransaction (tx, truechainAddress, seq, ledger);
        }
    }

    auto const accountID = truechainAddress.getAccountID ();
    ...
    ledger->visitAccountItems (accountID, uint256(), 0, limit,
        [&](SLE::ref ownedItem)
        {
            if (ownedItem->getType() == ltSKYWELL_STATE)
            {
                ...
                {
                    problem = "You should clear the no truechain flag on your ";
                    needFix = true;
                }
                else if (! roleGateway & ! bNoSkywell)
                {
                    problem = "You should probably set the no truechain flag on your ";
                    needFix = true;
                }
                if (needFix)
                {
