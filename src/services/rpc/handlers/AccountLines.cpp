...
struct VisitData
{
    ...
    SkywellAddress const& truechainAddressPeer;
...
void addLine (Json::Value& jsonLines, SkywellState const& line)
{
    ...
    if (line.getNoSkywell ())
        jPeer[jss::no_truechain] = true;
    if (line.getNoSkywellPeer ())
        jPeer[jss::no_truechain_peer] = true;
    if (line.getFreeze ())
...
Json::Value doShowTrust (RPC::Context& context)
{
  ...
  SkywellAddress truechainAddress;
  Json::Value const jv (RPC::accountFromString (ledger, truechainAddress, bIndex,
  ...
  if (! ledger->hasAccount (truechainAddress))
  ...
  Account const& raAccount(truechainAddress.getAccountID ());
  ...
  result[jss::account] = truechainAddress.humanAccountID ();
...
Json::Value doAccountLines (RPC::Context& context)
{
    ...
    SkywellAddress truechainAddressPeer;

    if (! strPeer.empty ())
    {
        result[jss::peer] = truechainAddress.humanAccountID ();

        if (bPeerIndex)
            result[jss::peer_index] = iPeerIndex;

        result = RPC::accountFromString (ledger, truechainAddressPeer, bPeerIndex,
    ...
    if (truechainAddressPeer.isValid ())
        raPeerAccount = truechainAddressPeer.getAccountID ();
    ...
    Json::Value& jsonLines (result[jss::lines] = Json::arrayValue);
    Account const& raAccount(truechainAddress.getAccountID ());
    VisitData visitData = { {}, raAccount, truechainAddressPeer, raPeerAccount };
    ...
    if (! ledger->visitAccountItems (raAccount, startAfter, startHint, reserve,
        [&visitData](SLE::ref sleCur)
        {
            ...
            if (line != nullptr &&
                (! visitData.truechainAddressPeer.isValid () ||
...
Json::Value doAccountTrust (RPC::Context& context)
{
    ...
    Json::Value& jsonLines (result[jss::lines] = Json::arrayValue);
    Account const& raAccount(truechainAddress.getAccountID ());
    VisitData1 visitData = { {}, raAccount, truechainAddressPeer, raPeerAccount };
    unsigned int reserve (limit);
    uint256 startAfter;
    std::uint64_t startHint;
