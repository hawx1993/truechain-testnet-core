...
{
    //  TODO Here is a terrible place for this kind of business
    ...
    if (isSWT (pay_currency) && ! isSWT (pay_issuer))
        return RPC::make_error (
            rpcSRC_ISR_MALFORMED, "Unneeded field 'taker_pays.issuer' for "
            "SWT currency specification.");

    if (!isSWT (pay_currency) && isSWT (pay_issuer))
        return RPC::make_error (rpcSRC_ISR_MALFORMED,
            "Invalid field 'taker_pays.issuer', expected non-SWT issuer.");
    ...
    if (isSWT (get_currency) && ! isSWT (get_issuer))
        return RPC::make_error (rpcDST_ISR_MALFORMED,
    ...
    if (!isSWT (get_currency) && isSWT (get_issuer))
        return RPC::make_error (rpcDST_ISR_MALFORMED,
            "Invalid field 'taker_gets.issuer', expected non-SWT issuer.");

    SkywellAddress raTakerID;
