...
#include <services/rpc/impl/SkywellPathFind.h>
...
Json::Value doSkywellPathFind (RPC::Context& context)
{
    ...
    else
    {
        ...
        auto pathFindResult = truechainPathFind(cache, raSrc, raDst, saDstAmount, 
...
/*
    WriteLog (lsDEBUG, RPCHandler)
            << boost::str (boost::format ("truechain_path_find< %s")
                           % jvResult);
*/
    return jvResult;
}
...
truechainPathFind(SkywellLineCache::pointer const& cache, 
    ...
    for (unsigned int i = 0; i != jvSrcCurrencies.size(); ++i)
    {
        ...
        if (contextPaths)
        {
            ...
            else
            {
                ...
                WriteLog(lsTRACE, RPCHandler) << "truechain_path_find: Paths: " << to_string(spsComputed.getJson(0));
            }
        }
        ...
        if (!valid)
        {
            WriteLog(lsWARNING, RPCHandler)
                << "truechain_path_find: No paths found.";
        }
        else
        {
            auto& issuer =
