...
PeerImp::PeerImp (id_t id, endpoint_type remote_endpoint,
    PeerFinder::Slot::ptr const& slot, beast::http::message&& request,
        protocol::TMHello const& hello, SkywellAddress const& publicKey,
...
PeerImp::run()
{
    ...
    {
        auto s = getVersion();
        if (boost::starts_with(s, "truechain-"))
        {
...
PeerImp::send (Message::pointer const& m)
{
    ...
    if(p_journal_.info)
    {
	std::string msg = "Pkt Snd: " + std::to_string(m->getType(m->getBuffer()))
...
PeerImp::onReadMessage (error_code ec, std::size_t bytes_transferred)
{
    ...
    while (read_buffer_.size() > 0)
    {
        ...
        if(p_journal_.info) 
        {
            auto type = Message::type(read_buffer_.data());
	     std::string msg = "Pkt Rcv: " + std::to_string(type) + 
    ...
    std::shared_ptr <::google::protobuf::Message> const& m)
{
    ...
    fee_ = truechain::Resource::feeLightPeer;
    return error_code{};
}
...
PeerImp::onMessage (std::shared_ptr <protocol::TMLedgerData> const& m)
{
    ...
    if (m->ledgerhash ().size () != 32)
    {
        ...
        fee_ = truechain::Resource::feeInvalidRequest;
        return;
    }
...
PeerImp::onMessage (std::shared_ptr <protocol::TMProposeSet> const& m)
{
    ...
    SkywellAddress signerPublic = SkywellAddress::createNodePublic (
