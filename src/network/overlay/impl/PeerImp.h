...
#ifndef SKYWELL_OVERLAY_PEERIMP_H_INCLUDED
#define SKYWELL_OVERLAY_PEERIMP_H_INCLUDED

#include <ledger/LedgerProposal.h>
#include <common/base/Log.h> // deprecated
...
public:
    ...
    */
    ...
    enum class State
    {
        ...
        /** Running the Skywell protocol actively */
        ,active
    };
...
public:
        ...
        Buffers const& buffers, PeerFinder::Slot::ptr&& slot,
            beast::http::message&& response, Resource::Consumer usage,
                protocol::TMHello&& hello,
                    SkywellAddress const& legacyPublicKey, id_t id,
                        OverlayImpl& overlay);
    ...
    */
    ...
    SkywellAddress const&
    getNodePublic () const override
    {
        return publicKey_;
    }

    /** Return the version of truechain that the peer is running, if reported. */
    std::string
    getVersion() const;
    ...
    for (;first != last; ++first)
    {
        ...
        if (ep.address.is_v4())
            tme.mutable_ipv4()->set_ipv4(
                htobe32(ep.address.to_v4().value)
                );
        else
