...
OverlayImpl::onPrepare()
{
    ...
    // If no IPs are specified, use the Skywell Labs round robin
    // pool to get some servers to insert into the boot cache.
    if (bootstrapIps.empty ())
        bootstrapIps.push_back ("r.truechain.com 51235");
    ...
    // Add the ips_fixed from the truechain.cfg file
...
OverlayImpl::onPeerDeactivate (Peer::id_t id,
    SkywellAddress const& publicKey)
{
    std::lock_guard <decltype(mutex_)> lock (mutex_);
...
/** The number of active peers on the network
    ...
    and are running the Skywell protocol.
*/
std::size_t
OverlayImpl::size()
{
