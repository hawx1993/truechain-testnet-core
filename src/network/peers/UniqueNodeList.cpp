...
public:
    ...
    void nodeAddPublic (SkywellAddress const& naNodePublic, ValidatorSource vsWhy, std::string const& strComment)
    {
        seedNode    snCurrent;
    ...
    // Queue a domain for a single attempt fetch a truechain.txt.
    ...
    void nodeRemovePublic (SkywellAddress const& naNodePublic)
    {
        {
    ...
    bool nodeInUNL (SkywellAddress const& naNodePublic)
    {
        ScopedUNLLockType sl (mUNLLock);
    ...
    bool nodeInCluster (SkywellAddress const& naNodePublic)
    {
        ScopedUNLLockType sl (mUNLLock);
    ...
    bool nodeInCluster (SkywellAddress const& naNodePublic, std::string& name)
    {
        ScopedUNLLockType sl (mUNLLock);
        std::map<SkywellAddress, ClusterNodeStatus>::iterator it = m_clusterNodes.find (naNodePublic);
    ...
    bool nodeUpdate (SkywellAddress const& naNodePublic, ClusterNodeStatus const& cnsStatus)
    {
        ScopedUNLLockType sl (mUNLLock);
    ...
    std::map<SkywellAddress, ClusterNodeStatus> getClusterStatus ()
    {
        std::map<SkywellAddress, ClusterNodeStatus> ret;
        {
            ScopedUNLLockType sl (mUNLLock);
            ret = m_clusterNodes;
        }
    ...
    std::uint32_t getClusterFee ()
    {
        ...
        std::vector<std::uint32_t> fees;
        {
            ScopedUNLLockType sl (mUNLLock);
            {
                for (std::map<SkywellAddress, ClusterNodeStatus>::iterator it = m_clusterNodes.begin(),
    ...
    void nodeBootstrap ()
    {
        ...
        // Always load from truechain.cfg
        if (!getConfig ().validators.empty ())
        {
            SkywellAddress   naInvalid;  // Don't want a referrer on added entries.
...
private:
    ...
    void trustedLoad ()
    {
        ...
        for (auto const& c : getConfig ().CLUSTER_NODES)
        {
            ...
            {
                SkywellAddress a = SkywellAddress::createNodePublic (match[1]);
    ...
    // Given a truechain.txt, process it.
    //
    ...
    // Try to process the next fetch of a truechain.txt.
    void fetchNext ()
    {
        bool    bFull;
    ...
    // Get the truechain.txt and process it.
    ...
    void getValidatorsUrl (SkywellAddress const& naNodePublic,
    ...
    void getIpsUrl (SkywellAddress const& naNodePublic, IniFileSections secSite)
    {
    ...
    bool responseIps (std::string const& strSite, SkywellAddress const& naNodePublic, const boost::system::error_code& err, int iStatus, std::string const& strIpsFile)
    {
        bool    bReject = !err && iStatus != 200;
    ...
    // After fetching a truechain.txt from a web site, given a IniFileSections with validators, parse and persist it.
    bool responseValidators (std::string const& strValidatorsUrl, SkywellAddress const& naNodePublic, IniFileSections secSite, std::string const& strSite, const boost::system::error_code& err, int iStatus, std::string const& strValidatorsFile)
    {
        bool    bReject = !err && iStatus != 200;
    ...
    void processIps (std::string const& strSite, SkywellAddress const& naNodePublic, IniFileSections::mapped_type* pmtVecStrIps)
    {
    ...
    int processValidators (std::string const& strSite, std::string const& strValidatorsSrc, SkywellAddress const& naNodePublic, ValidatorSource vsWhy, IniFileSections::mapped_type* pmtVecStrValidators)
    {
    ...
    // Process a truechain.txt.
    void processFile (std::string const& strDomain, SkywellAddress const& naNodePublic, IniFileSections secSite)
    {
        //
        // Process Validators
        //
    ...
    bool getSeedNodes (SkywellAddress const& naNodePublic, seedNode& dstSeedNode)
    {
        std::string strSql =
                str (boost::format (
...
private:
    ...
    std::map<SkywellAddress, ClusterNodeStatus> m_clusterNodes;
