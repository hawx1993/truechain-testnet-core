...
#ifndef SKYWELL_APP_LEDGER_LEDGER_H_INCLUDED
#define SKYWELL_APP_LEDGER_LEDGER_H_INCLUDED
...
public:
    ...
    Ledger (const SkywellAddress & masterID, std::uint64_t startAmount);
    ...
    bool hasAccount (const SkywellAddress & acctID) const;
    AccountState::pointer getAccountState (const SkywellAddress & acctID) const;
	AccountState::pointer getAccountState1(const SkywellAddress & acctID) const;
    LedgerStateParms writeBack (LedgerStateParms parms, SLE::ref);
    SLE::pointer getAccountRoot (Account const& accountID) const;
    SLE::pointer getAccountRoot (const SkywellAddress & naAccountID) const;
    void updateSkipList ();
    ...
    //
    // Skywell functions : credit lines
    //

    SLE::pointer
    getSkywellState (uint256 const& uNode) const;
...
private:
    ...
    // Skywell cost of the reference transaction
    ...
    typedef SkywellMutex StaticLockType;
