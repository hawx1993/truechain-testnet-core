...
#ifndef SKYWELL_APP_LEDGER_LEDGERENTRYSET_H_INCLUDED
#define SKYWELL_APP_LEDGER_LEDGERENTRYSET_H_INCLUDED
...
private:
    ...
    bool threadTx (
        const SkywellAddress & threadTo, Ledger::ref ledger,
        NodeToLedgerEntry& newMods);
    ...
    STAmount truechainHolds (
        Account const& account, Currency const& currency,
        Account const& issuer, FreezeHandling zeroIfFrozen);

    STAmount truechainTransferFee (
...
std::uint32_t
truechainTransferRate(LedgerEntrySet& ledger, Account const& issuer, Currency const& currency = noCurrency());
