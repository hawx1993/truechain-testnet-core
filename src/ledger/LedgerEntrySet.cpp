...
Json::Value LedgerEntrySet::getJson (int) const
{
    ...
    for (auto it = mEntries.begin (), end = mEntries.end (); it != end; ++it)
    {
        ...
        case ltSKYWELL_STATE:
            entry[jss::type] = "truechain_state";
            break;
...
bool LedgerEntrySet::threadTx (SkywellAddress const& threadTo, Ledger::ref ledger,
                               NodeToLedgerEntry& newMods)
{
...
STAmount LedgerEntrySet::truechainHolds (
    ...
    FreezeHandling zeroIfFrozen)
{
    ...
    {
        saAmount = truechainHolds (account, currency, issuer, zeroIfFrozen);
...
TER LedgerEntrySet::relationCreate (
    ...
    uint256 const&  uIndex,             // --> truechain state entry
...
{
	WriteLog(lsTRACE, LedgerEntrySet)
		<< "signCreate: " << to_string(uSrcAccountID);
...
TER LedgerEntrySet::trustCreate (
    ...
    const bool      bNoSkywell,          // --> others cannot truechain through
    ...
    Account const& uHighAccountID)
{
    ...
    WriteLog (lsTRACE, LedgerEntrySet) << "trustDelete: Deleting truechain line: low";
    terResult   = dirDelete (
        false,
        uLowNode,
        getOwnerDirIndex (uLowAccountID),
    ...
    {
        WriteLog (lsTRACE, LedgerEntrySet)
                << "trustDelete: Deleting truechain line: high";
        terResult   = dirDelete (
            false,
            uHighNode,
            getOwnerDirIndex (uHighAccountID),
    ...
    WriteLog (lsTRACE, LedgerEntrySet) << "trustDelete: Deleting truechain line: state";
...
TER LedgerEntrySet::truechainCredit (
    ...
    STAmount const& saAmount, bool bCheckIssuer)
{
    ...
    if (!sleSkywellState)
    {
        ...
        WriteLog (lsDEBUG, LedgerEntrySet) << "truechainCredit: "
    ...
    else
    {
        ...
        WriteLog (lsTRACE, LedgerEntrySet) << "truechainCredit: " <<
            to_string (uSenderID) <<
            " -> " << to_string (uReceiverID) <<
...
STAmount LedgerEntrySet::truechainTransferFee (
    Account const& from,
    Account const& to,
    Account const& issuer,
    STAmount const& saAmount)
{
    ...
    {
        std::uint32_t uTransitRate = truechainTransferRate (*this, issuer,saAmount.getCurrency());
        ...
        if (QUALITY_ONE != uTransitRate)
        {
            ...
            WriteLog (lsDEBUG, LedgerEntrySet) << "truechainTransferFee:" <<
...
TER LedgerEntrySet::truechainSend (
    Account const& uSenderID, Account const& uReceiverID,
    STAmount const& saAmount, STAmount& saActual)
{
    ...
    if (uSenderID == issuer || uReceiverID == issuer || issuer == noAccount())
    {
        ...
        terResult   = truechainCredit (uSenderID, uReceiverID, saAmount, false);
        saActual    = saAmount;
        terResult   = tesSUCCESS;
    }
    else
    {
        ...
        STAmount saTransitFee = truechainTransferFee (
        ...
        WriteLog (lsDEBUG, LedgerEntrySet) << "truechainSend> " <<
            to_string (uSenderID) <<
            " - > " << to_string (uReceiverID) <<
        ...
        terResult   = truechainCredit (issuer, uReceiverID, saAmount);

        if (tesSUCCESS == terResult)
            terResult   = truechainCredit (uSenderID, issuer, saActual);
    }
    ...
    STAmount const& saAmount)
{
    ...
    if (!saAmount.isNative ())
    {
        ...
        return truechainSend (uSenderID, uReceiverID, saAmount, saActual);
    }
...
std::uint32_t
truechainTransferRate (LedgerEntrySet& ledger, Account const& issuer,Currency const& currency)
{
...
std::uint32_t
truechainTransferRate (LedgerEntrySet& ledger, Account const& uSenderID,
Account const& uReceiverID, Account const& issuer, Currency const& currency)
{
    ...
    return (uSenderID == issuer || uReceiverID == issuer)
           ? QUALITY_ONE
           : truechainTransferRate (ledger, issuer,currency);
}
