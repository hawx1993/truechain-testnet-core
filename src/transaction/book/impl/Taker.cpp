...
BasicTaker::Rate::divide (Amount const& amount) const
{
    ...
    return truechain::divide (amount, rate_, amount.issue ());
}
...
BasicTaker::Rate::multiply (Amount const& amount) const
{
    ...
    return truechain::multiply (amount, rate_, amount.issue ());
}
    ...
    , cross_type_ (cross_type)
{
    ...
    assert (cross_type != CrossType::XrpToIou ||
        (isSWT (issue_in ()) && !isSWT (issue_out ())));
...
{
    return isSWT (issuer) || (account == issuer)
        ? QUALITY_ONE
        : truechainTransferRate (view, issuer);
}
