...
Pathfinder::Pathfinder (
    ...
    :   mSrcAccount (uSrcAccount),
        ...
        mEffectiveDst (isSWT(saDstAmount.getIssuer ()) ?
...
bool Pathfinder::findPaths (int searchLevel)
{
    ...
    bool useIssuerAccount
            = mSrcIssuer && !currencyIsSWT && !isSWT (*mSrcIssuer);
    ...
    if (!mLedger->getSLEi (getAccountRootIndex (mDstAccount)))
    {
        ...
        {
            WriteLog (lsDEBUG, Pathfinder)
                    << "New account not being funded in SWT ";
            return false;
        }
    ...
    uint64_t& qualityOut) const    // OUT: The returned initial quality
{
    ...
    try
    {
        ...
        auto rc = path::SkywellCalc::truechainCalculate (
            lesSandbox,
            mSrcAmount,
            minDstAmount,
            mDstAccount,
        ...
        rc = path::SkywellCalc::truechainCalculate (
            lesSandbox,
            mSrcAmount,
...
static bool isDefaultPath (STPath const& path)
{
    ...
    // https://forum.truechain.com/viewtopic.php?f=2&t=8206&start=10#p57713
    //
    // JoelKatz writes:
    ...
    Account const& dstAccount)
{
    ...
    if (!bFrozen)
    {
        ...
        for (auto const& item : mRLCache->getSkywellLines (account))
        {
            ...
            else if (rspEntry->getNoSkywellPeer ())
            {
...
bool Pathfinder::isNoSkywell (
    Account const& fromAccount,
    Account const& toAccount,
    Currency const& currency)
{
    auto sleSkywell = mLedger->getSLEi (
        getSkywellStateIndex (toAccount, fromAccount, currency));

    auto const flag ((toAccount > fromAccount)
                     ? lsfHighNoSkywell : lsfLowNoSkywell);

    return sleSkywell && (sleSkywell->getFieldU32 (sfFlags) & flag);
}

// Does this path end on an account-to-account link whose last account has
// set "no truechain" on the link?
bool Pathfinder::isNoSkywellOut (STPath const& currentPath)
{
    ...
    // no truechain on the output. A path with no truechain on its output can't be
    // followed by a link with no truechain on its input.
    auto const& fromAccount = (currentPath.size () == 1)
        ? mSrcAccount
        : (currentPath.end () - 2)->getAccountID ();
    auto const& toAccount = endElement.getAccountID ();
    return isNoSkywell (fromAccount, toAccount, endElement.getCurrency ());
}
    ...
    if (addFlags & afADD_ACCOUNTS)
    {
        ...
        else
        {
            ...
            if (sleEnd)
            {
                ...
                bool const bIsNoSkywellOut (
                    isNoSkywellOut (currentPath));
                bool const bDestOnly (
                    addFlags & afAC_LAST);

                auto& truechainLines (mRLCache->getSkywellLines (uEndAccount));

                AccountCandidates candidates;
                candidates.reserve (truechainLines.size ());

                for (auto const& item : truechainLines)
                {
                    auto* rs = dynamic_cast<SkywellState const *> (item.get ());
                    if (!rs)
                    {
                        WriteLog (lsERROR, Pathfinder)
                                << "Couldn't decipher SkywellState";
                        continue;
                    }
                        ...
                        !currentPath.hasSeen (acct, uEndCurrency, acct))
                    {
                        ...
                        else if (bIsNoSkywellOut && rs->getNoSkywell ())
                        {
