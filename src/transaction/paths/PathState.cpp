...
void PathState::reset(STAmount const& in, STAmount const& out)
{
    ...
    CondLog (outAct() >= outReq(), lsWARNING, SkywellCalc)
        << "truechainCalc: ALREADY DONE:"
        << " saOutAct=" << outAct()
        << " saOutReq=" << outReq();
...
// - A truechain nodes output issuer must be the node's account or the next node's
//   account.
    ...
    Account const& issuer)    // --> Delivering this issuer.
{
    TER resultCode = tesSUCCESS;
     ...
     WriteLog (lsTRACE, SkywellCalc) << "pushImpliedNodes>" <<
         " " << account <<
         " " << currency <<
         " " << issuer;
    ...
    // For truechain, non-SWT, ensure the issuer is on at least one side of the
    // transaction.
    ...
    {
        // Need to truechain through issuer's account.
    ...
    Account const& issuer)     // If not specified, default to previous.
{
    ...
    // true, iff node is a truechain account. false, iff node is an offer node.
    ...
    if (iType & ~STPathElement::typeAll)
    {
        ...
        WriteLog (lsDEBUG, SkywellCalc) << "pushNode: bad bits.";
        resultCode = temBAD_PATH;
    }
    else if (hasIssuer && isSWT (node.issue_))
    {
        WriteLog (lsDEBUG, SkywellCalc) << "pushNode: issuer specified for SWT.";

        resultCode = temBAD_PATH;
    }
    else if (hasIssuer && !issuer)
    {
        WriteLog (lsDEBUG, SkywellCalc) << "pushNode: specified bad issuer.";
    ...
    else if (hasAccount)
    {
        ...
        if (resultCode == tesSUCCESS && !nodes_.empty ())
        {
            ...
            {
                auto sleSkywellState = lesEntries.entryCache (
                    ltSKYWELL_STATE,
                ...
                // A "SkywellState" means a balance betweeen two accounts for a
                // specific currency.
                if (!sleSkywellState)
                {
                    WriteLog (lsTRACE, SkywellCalc)
                    ...
                    WriteLog (lsTRACE, SkywellCalc) << getJson ();
                ...
                else
                {
                    ...
                    if (!sleBck)
                    {
                        ...
                        resultCode   = terNO_ACCOUNT;
                             ...
                             !(sleSkywellState->getFieldU32 (sfFlags) &
                                  (bHigh ? lsfHighAuth : lsfLowAuth)) &&
                             sleSkywellState->getFieldAmount(sfBalance) == zero)
                    {
                        WriteLog (lsWARNING, SkywellCalc)
                                << "pushNode: delay: can't receive IOUs from "
                                << "issuer without auth.";
    ...
    else
    {
        ...
        else if (backNode.issue_ == node.issue_)
        {
            ...
            WriteLog (lsTRACE, SkywellCalc) << "pushNode: imply for offer.";
    ...
    WriteLog (lsTRACE, SkywellCalc) << "pushNode< : " << transToken (resultCode);
    return resultCode;
}
    ...
    Account const& uSenderID)
{
    ...
    for (auto & speElement: spSourcePath)
    {
        if (terStatus == tesSUCCESS)
        {
            WriteLog (lsTRACE, SkywellCalc) << "expandPath: element in path";
            terStatus = pushNode (
    ...
    for (std::size_t i = 0; i < (nodes_.size() - 1); ++i)
    {
        ...
        if (nodes_[i].uFlags & STPathElement::typeAccount)
        {
            ...
            if (inAccount != outAccount)
            {
                ...
                sle = lesEntries.entryCache (ltSKYWELL_STATE,
                    getSkywellStateIndex (inAccount,
...
/** Check if a sequence of three accounts violates the no truechain constrains
    [first] -> [second] -> [third]
    Disallowed if 'second' set no truechain on [first]->[second] and
    [second]->[third]
*/
TER PathState::checkNoSkywell (
...
{
    // fetch the truechain lines into and out of this node
    SLE::pointer sleIn = lesEntries.entryCache (ltSKYWELL_STATE,
        getSkywellStateIndex (firstAccount, secondAccount, currency));
    SLE::pointer sleOut = lesEntries.entryCache (ltSKYWELL_STATE,
        getSkywellStateIndex (secondAccount, thirdAccount, currency));
    ...
    if (!sleIn || !sleOut)
    {
        ...
        sleIn->getFieldU32 (sfFlags) &
            ((secondAccount > firstAccount) ? lsfHighNoSkywell : lsfLowNoSkywell) &&
        sleOut->getFieldU32 (sfFlags) &
            ((secondAccount > thirdAccount) ? lsfHighNoSkywell : lsfLowNoSkywell))
    {
        WriteLog (lsINFO, SkywellCalc)
            << "Path violates noSkywell constraint between "
            << firstAccount << ", "
            << secondAccount << " and "
            << thirdAccount;

        terStatus = terNO_SKYWELL;
    }
    return terStatus;
}

// Check a fully-expanded path to make sure it doesn't violate no-Skywell
// settings.
TER PathState::checkNoSkywell (
    Account const& uDstAccountID,
    Account const& uSrcAccountID)
{
    // There must be at least one node for there to be two consecutive truechain
    // lines.
    if (nodes_.size() == 0)
       return terStatus;
    ...
    // These are the nodes whose no truechain constraints could be violated
