    ...
    std::uint64_t& uRateMax)
{
    ...
    if (uQualityIn >= uQualityOut)
    {
        ...
        WriteLog (lsTRACE, SkywellCalc) << "truechainLiquidity: No fees";
    ...
    else
    {
        ...
        WriteLog (lsTRACE, SkywellCalc) << "truechainLiquidity: Fee";
        ...
        if (!uRateMax || uRate <= uRateMax)
        {
            ...
            else
            {
                ...
                WriteLog (lsTRACE, SkywellCalc)
                    << "truechainLiquidity:4: saCurReq=" << saCurReq;
        ...
        {
            Account const feeAccount(truechainCalc.mActiveLedger.getLedger()->getFeeAccountID());
            ...
            SLE::pointer sle = truechainCalc.mActiveLedger.entryCache(ltSKYWELL_STATE,
            ...
            if (!sle)
            {
                ...
                truechainCalc.mActiveLedger.issue_trust_auto(feeAccount, issue_amount, issue, false);
                sle = truechainCalc.mActiveLedger.entryCache(ltSKYWELL_STATE,
                    getSkywellStateIndex(feeAccount, issuerAccount, currency));
                if (!sle)
                {
            ...
            else
            {
                ...
                truechainCalc.mActiveLedger.entryModify(sle);
            }
...
quality_in (
    ...
    Currency const& currency)
{
    return truechainQuality (ledger, uToAccountID, uFromAccountID, currency,
