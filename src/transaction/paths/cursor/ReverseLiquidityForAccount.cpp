...
;
    ...
    //WriteLog (lsTRACE, SkywellCalc) << pathState_.getJson ();
    ...
    // page section: https://truechain.com/wiki/Transit_Fees#Implementation
        ...
        else
        {
                ...
                // Did not complete redeeming previous IOUs.
            {
                ...
                truechainLiquidity (
                    truechainCalc_,
                    QUALITY_ONE,
                    truechainTransferRate(ledger(), node().account_, saCurIssueAct.getCurrency()),
                    saPrvRedeemReq,
                    node().saRevIssue,
                    previousNode().saRevRedeem,
                    saCurIssueAct,
    ...
    else if (previousNodeIsAccount && !nextNodeIsAccount)
    {
            ...
            && node().saRevDeliver)                 // Need some issued.
        {
            ...
            truechainLiquidity (
                truechainCalc_,
                QUALITY_ONE,
                truechainTransferRate(ledger(), node().account_, saCurDeliverAct.getCurrency()),
                saPrvRedeemReq,
                node().saRevDeliver,
                previousNode().saRevRedeem,
                saCurDeliverAct,
    ...
    else if (!previousNodeIsAccount && nextNodeIsAccount)
    {
        if (isFinalNode)
        {
            ...
            if (saCurWantedReq <= zero)
            {
                ...
                WriteLog (lsFATAL, SkywellCalc) << "CurWantReq was not positive";
                return tefEXCEPTION;
            }
        ...
        else
        {
            ...
            // deliver -> redeem
            // TODO(tom): now we have more checking in nodeSkywell, these checks
            // might be redundant.
