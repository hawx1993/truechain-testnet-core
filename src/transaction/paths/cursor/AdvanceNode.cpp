...
TER PathCursor::advanceNode (STAmount const& amount, bool reverse) const
{
    ...
    PathCursor withMultiQuality {truechainCalc_, pathState_, multi, nodeIndex_};
...
TER PathCursor::advanceNode (bool const bReverse) const
{
    ...
    do
    {
        ...
        {
            WriteLog (lsWARNING, SkywellCalc) << "Loop count exceeded";
            return tefEXCEPTION;
        }
        ...
        if (!node().bEntryAdvance)
        {
            ...
            {
                WriteLog (lsTRACE, SkywellCalc) << "advanceNode: as is";
            }
        }
        ...
        else
        {
            ...
            else
            {
                     ...
                     getParentCloseTimeNC ()))
                {
                    ...
                    truechainCalc_.permanentlyUnfundedOffers_.insert(
                        node().offerIndex_);
                    continue;
                }
                ...
                if (node().saTakerPays <= zero || node().saTakerGets <= zero)
                {
                    ...
                    else if (truechainCalc_.permanentlyUnfundedOffers_.find (index)
                             != truechainCalc_.permanentlyUnfundedOffers_.end ())
                    {
                ...
                auto itPast = truechainCalc_.mumSource_.find (accountIssue);
                bool bFoundPast = (itPast != truechainCalc_.mumSource_.end ());
                ...
                if (node().saOfferFunds <= zero)
                {
                    ...
                    if (bReverse && !bFoundReverse && !bFoundPast)
                    {
                        ...
                        // still do deletions.
                        // Mark offer for always deletion.
                        truechainCalc_.permanentlyUnfundedOffers_.insert (node().offerIndex_);
                    }
                    else
                    {
