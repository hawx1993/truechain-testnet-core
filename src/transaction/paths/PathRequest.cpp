...
bool PathRequest::isValid (SkywellLineCache::ref crCache)
{
    ScopedLockType sl (mLock);
    ...
    if (bValid)
    {
        ...
        else
        {
            ...
            {
                bool const disallowSWT (
                    asDst->peekSLE ().getFlags() & lsfDisallowSWT);

                auto usDestCurrID = accountDestCurrencies (
                        raDstAccount, crCache, !disallowSWT);
...
Json::Value PathRequest::doUpdate (SkywellLineCache::ref cache, bool fast)
{
    ...
    for (auto const& currIssuer: sourceCurrencies)
    {
        ...
        if (valid)
        {
            ...
            auto& sourceAccount = !isSWT (currIssuer.account)
                    ? currIssuer.account
                    : isSWT (currIssuer.currency)
                        ? xrpAccount()
                        : raSrcAccount.getAccountID ();
            ...
            saMaxAmount.negate ();
            m_journal.debug << iIdentifier
                            << " Paths found, calling truechainCalc";
            ...
            {
                m_journal.debug << iIdentifier << " truechainCalc returns "
                    << transHuman (rc.result ());
            }
        }
        else
