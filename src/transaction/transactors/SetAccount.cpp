...
public:
    ...
    TER preCheck () override
    {
        ...
        // DisallowSWT
        //
        bool bSetDisallowSWT   = (uTxFlags & tfDisallowSWT) || (uSetFlag == asfDisallowSWT);
        bool bClearDisallowSWT = (uTxFlags & tfAllowSWT) || (uClearFlag == asfDisallowSWT);

        if (bSetDisallowSWT && bClearDisallowSWT)
        {
            m_journal.trace << "Malformed transaction: Contradictory flags set.";
            return temINVALID_FLAG;
        }
    ...
    TER doApply () override
    {
        ...
        // DisallowSWT
        //
        if (bSetDisallowSWT && !(uFlagsIn & lsfDisallowSWT))
        {
            m_journal.trace << "Set lsfDisallowSWT.";
            uFlagsOut |= lsfDisallowSWT;
        }

        if (bClearDisallowSWT && (uFlagsIn & lsfDisallowSWT))
        {
            m_journal.trace << "Clear lsfDisallowSWT.";
            uFlagsOut &= ~lsfDisallowSWT;
        }
        ...
        // DefaultSkywell
        //
        if (uSetFlag == asfDefaultSkywell)
        {
            uFlagsOut   |= lsfDefaultSkywell;
        }
        else if (uClearFlag == asfDefaultSkywell)
        {
            uFlagsOut   &= ~lsfDefaultSkywell;
        }
