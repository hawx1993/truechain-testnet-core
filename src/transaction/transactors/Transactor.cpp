...
TER transact_SetREL(STTx const& txn, TransactionEngineParams params, TransactionEngine* engine);
...
TER transact_Manage(STTx const& txn, TransactionEngineParams params, TransactionEngine* engine);
TER transact_AccountMerge(STTx const& txn, TransactionEngineParams params, TransactionEngine* engine);
TER transact_SetSign(STTx const& txn, TransactionEngineParams params, TransactionEngine* engine);
TER transact_SetNickName(STTx const& txn, TransactionEngineParams params, TransactionEngine* engine);
TER transact_Message(STTx const& txn, TransactionEngineParams params, TransactionEngine* engine);
...
TransactionEngine* engine)
{
    ...
    case ttREL_SET:
    case ttREL_DEL:
        return transact_SetREL(txn, params, engine);
	...
	case ttNICKNAME_SET:
		return  transact_SetNickName(txn, params, engine);

    case ttACCOUNT_MERGE:
        return transact_AccountMerge(txn, params, engine);

    case ttMNGFEE:
    case ttBLKLISTSET:
    case ttBLKLISTRMV:
    case ttMNGISSUER:
        return transact_Manage(txn, params, engine);
        
    case ttSIGN_SET:
        return transact_SetSign(txn, params, engine);
    case ttMESSAGE:
        return transact_Message(txn, params, engine);

	case ttOPERATION:
		return transact_Operations(txn, params, engine);
...
{	
    ...
    if(getApp().getLedgerMaster ().getClosedLedger()->checkBlackList(mTxnAccountID))
    {
       ...
       m_journal.warning << "BlackList transaction: " <<
             "from blacklist account " << to_string (mTxnAccountID) ;
	   return telBLKLIST;
    }
    // Extract signing key
...
TER Transactor::apply ()
{
	...
	mFeeAccount = mEngine->view().entryCache(ltACCOUNT_ROOT,
		getAccountRootIndex(mFeeAccountID));
